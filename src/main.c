...
/* Buttons: use sw0/sw1/sw2 aliases (portable across Nordic DKs) */
...
static bool want_advertising = false;
static bool adv_is_running = false;
...
/* LED0: advertising, LED1: connected, LED2: RPA mode */
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    ...
    /* If user wants it, resume advertising (once) */
    if (want_advertising) {
...
/* ---- Pairing/Bonding (Just Works) ---- */
static void pairing_confirm(struct bt_conn *conn)
{
    LOG_INF("Pairing confirm -> accepting (Just Works)");
